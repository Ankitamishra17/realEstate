...
const STEPS = [
  { icon: Search,     step: "Step One",   title: "Browse & Shortlist",       text: "Filter by budget, location, and property type to build your shortlist." },
  { icon: MapPin,     step: "Step Two",   title: "Schedule a Site Visit",    text: "Your advisor arranges visits at a time that works for you, in person or virtual." },
  { icon: Wallet,     step: "Step Three", title: "Finalise & Negotiate",     text: "We help you negotiate fairly and explain every cost before you commit." },
  { icon: FileCheck2, step: "Step Four",  title: "Documentation & Loan",     text: "Legal verification, home loan assistance, and paperwork handled for you." },
  { icon: ShieldCheck,step: "Step Five",  title: "Registration & Handover",  text: "Property registration and key handover, completed start to finish." },
];

const BENEFITS = [
  { icon: ShieldCheck,  title: "RERA-Verified Listings",  desc: "Every project on this page is registered and legally verified before listing." },
  { icon: IndianRupee,  title: "Transparent Pricing",     desc: "No hidden charges — see the full cost breakup before you make an offer." },
  { icon: FileCheck2,   title: "End-to-End Paperwork",    desc: "From token agreement to registration, our legal team manages the process." },
  { icon: Percent,      title: "Home Loan Assistance",    desc: "Pre-approved tie-ups with leading banks to get you the best rate." },
];
...
function EyebrowLeft({ children }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
      ...
      <span style={{
        color: "#b8892e", fontSize: "clamp(9px,2.5vw,11px)",
        letterSpacing: "0.2em", fontWeight: 700, textTransform: "uppercase", whiteSpace: "nowrap",
      }}>
        {children}
      </span>
    </div>
...
function StatusPill({ status }) {
  ...
  return (
    <span style={{
      ...
      background: colorMap[status] || "#b8892e", color: "#fff",
      fontSize: 11, fontWeight: 700, letterSpacing: "0.03em",
      padding: "5px 12px", borderRadius: 2, zIndex: 2,
    }}>
      {status}
    </span>
  );
      ...
      <div className="listing-card" style={{
        ...
        <div style={{ position: "relative", height: 210, overflow: "hidden", flexShrink: 0 }}>
          ...
          <div style={{
            ...
            background: "linear-gradient(to top, rgba(18,36,61,0.55), transparent)",
          }} />
        </div>

        <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", flex: 1 }}>
          <p style={{ color: "#b8892e", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>
            {p.type}
          </p>
          ...
          <div style={{
            ...
            paddingTop: "0.875rem", borderTop: "1px solid #e2d9cc", marginBottom: "1rem",
          }}>
            {p.beds && (
...
export default function Buy() {
  ...
  return (
    <div style={{
      background: "#f7f4ef", color: "#12243d",
      fontFamily: "'Georgia', 'Times New Roman', serif", overflowX: "hidden",
    }}>
      <style>{`
      ...
      <section style={{
        background: "#12243d",
        padding: "clamp(5rem,8vw,7rem) clamp(1.25rem,4vw,2.5rem) clamp(3rem,6vw,5rem)",
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: 1200, margin: "0 auto" }}>
          <div className="buy-hero-grid">
            ...
            <Fade delay={150} from="right">
              <div className="hero-stats-row" style={{ display: "flex", gap: "0.875rem", flexWrap: "wrap", justifyContent: "flex-end", marginTop: 0 }}>
                {[
                  { v: "9",    l: "Listings Live" },
                  { v: "8.5%", l: "Loan Rate From" },
                  ...
                  <div key={s.l} style={{
                    ...
                    borderRadius: 2, textAlign: "center", minWidth: 90, flex: "1 1 auto",
                  }}>
                    <p style={{ color: "#b8892e", fontWeight: 800, fontSize: "clamp(1.2rem,3vw,1.6rem)", lineHeight: 1 }}>{s.v}</p>
      ...
      {/* ── FILTERS + LISTINGS ── */}
      <section style={{ padding: "clamp(3rem,7vw,5rem) clamp(1.25rem,4vw,2.5rem) clamp(3rem,8vw,6rem)", background: "#f7f4ef" }}>
            ...
            <div className="filter-header">
              ...
              <div className="filter-groups">
                <div className="filter-row">
                  {TYPE_FILTERS.map((f) => (
                    <button key={f} className="filter-btn" onClick={() => setActiveType(f)} style={{
                      border: activeType === f ? "1px solid #b8892e" : "1px solid #e2d9cc",
                      background: activeType === f ? "#b8892e" : "#fff",
                      color: activeType === f ? "#fff" : "rgba(18,36,61,0.65)",
                      fontWeight: 600, fontSize: "clamp(11px,1.4vw,12px)",
                      padding: "0.45rem 0.9rem", borderRadius: 20, cursor: "pointer", whiteSpace: "nowrap",
                    }}>
                      {f}
                    </button>
                  ))}
      ...
      {/* ── WHY BUY WITH US ── */}
      <section style={{ padding: "clamp(3rem,7vw,6rem) clamp(1.25rem,4vw,2.5rem)", background: "#12243d", position: "relative", overflow: "hidden" }}>
            ...
            {BENEFITS.map((b, i) => (
              <Fade key={b.title} delay={i * 90}>
                <div className="benefit-card" style={{
                  ...
                  <h3 style={{ color: "#fff", fontWeight: 700, marginBottom: "0.6rem", fontSize: "clamp(13px,1.5vw,15px)" }}>{b.title}</h3>
                  <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 13, lineHeight: 1.7, margin: 0 }}>{b.desc}</p>
                </div>
              </Fade>
            ))}
          </div>
      ...
      {/* ── EMI CALCULATOR ── */}
      <section style={{ padding: "clamp(3rem,7vw,6rem) clamp(1.25rem,4vw,2.5rem)", background: "#fff" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <Fade>
            <div className="calc-grid" style={{
              ...
              borderRadius: 2, padding: "clamp(1.5rem,3vw,2.5rem)",
            }}>
              {/* Sliders */}
              <div>
                <div style={{ marginBottom: "1.75rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <span style={{ fontSize: 13, color: "rgba(18,36,61,0.6)", fontWeight: 600 }}>Loan Amount</span>
                    <span style={{ fontSize: 13, color: "#12243d", fontWeight: 700 }}>
                ...
                <div style={{ marginBottom: "1.25rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <span style={{ fontSize: 13, color: "rgba(18,36,61,0.6)", fontWeight: 600 }}>Loan Tenure</span>
                    <span style={{ fontSize: 13, color: "#12243d", fontWeight: 700 }}>{tenure} Years</span>
                  </div>
                  <input type="range" min="5" max="30" step="1"
                    value={tenure} onChange={(e) => setTenure(Number(e.target.value))} />
                </div>
                <p style={{ fontSize: 12, color: "rgba(18,36,61,0.45)", lineHeight: 1.6 }}>
                  Calculated at {rate}% p.a. indicative interest rate. Actual rate may vary by lender.
                </p>
              </div>

              {/* Result */}
              <div style={{ background: "#12243d", borderRadius: 2, padding: "2rem 1.5rem", textAlign: "center" }}>
                <Calculator style={{ width: 26, height: 26, color: "#b8892e", margin: "0 auto 1rem" }} />
                <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>
      ...
      <section style={{ padding: "clamp(3rem,7vw,6rem) clamp(1.25rem,4vw,2.5rem)", background: "#f7f4ef" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Fade>
            <div style={{ textAlign: "center", marginBottom: "clamp(2rem,5vw,3.5rem)" }}>
          ...
          <div className="steps-grid" style={{ marginBottom: "clamp(2rem,4vw,3.5rem)" }}>
            {STEPS.map((s, i) => (
              <Fade key={s.title} delay={i * 80}>
                <div className="step-card" style={{
                  background: "#fff", border: "1px solid #e2d9cc", borderRadius: 2,
                  padding: "clamp(1.25rem,2vw,1.75rem) clamp(1rem,1.5vw,1.25rem)", height: "100%",
                }}>
                  <div style={{
                    width: 44, height: 44, borderRadius: 2,
                    background: "rgba(184,137,46,0.1)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: "#b8892e", marginBottom: "1rem",
                  }}>
                    <s.icon style={{ width: 20, height: 20 }} />
                  </div>
                  <p style={{ color: "#b8892e", fontSize: 10.5, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 6 }}>
                    {s.step}
                  </p>
                  <h3 style={{ color: "#12243d", fontWeight: 700, fontSize: "clamp(13px,1.4vw,14px)", marginBottom: 6, lineHeight: 1.35 }}>{s.title}</h3>
                  <p style={{ color: "rgba(18,36,61,0.55)", fontSize: 12.5, lineHeight: 1.65, margin: 0 }}>{s.text}</p>
                </div>
              </Fade>
            ))}
          </div>
          ...
          <Fade>
            <div style={{
              ...
              <h3 style={{ color: "#fff", fontWeight: 700, fontSize: "clamp(1.1rem,2.5vw,1.5rem)", marginBottom: 10 }}>
                Found Something You Like?
              </h3>
              <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, lineHeight: 1.7, maxWidth: 460, margin: "0 auto 1.5rem" }}>
