    ...
    <section className="w-full bg-[#2e5d42] py-20 t-6 sm:px-8 lg:px-16 text-[#fafaef]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          >
            Experience Ex cellence in Every Deal
          </h2>
        </motion.div>
        ...
        >
          {features.map((itemData, index) => (
            <motion.div
              key={index}
              variants={item}
              className="group relative  p-6 rounded-2xl backdrop-blur-lg bg-[#fafaef]/5 hover:bg-[#fafaef]/10 transition duration-300"
            >
              {/* Number */}
              ...
              {/* Hover Line Effect */}
              <div className="absolute bottom-0 left-0 w-0 h-[4px] bg-[#fafaef] rounded-full group-hover:w-full transition-all duration-300"></div>
            </motion.div>
          ))}
        </motion.div>
      </div>
